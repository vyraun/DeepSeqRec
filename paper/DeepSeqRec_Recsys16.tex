...
\maketitle
\begin{abstract}
Many recommendation settings have a sequential nature.  For example,
in online education tutoring, next problem recommendation that
maximizes learning should take a student's historical
progress into account.
Similarly, online web browsing is highly contextual and a user's site
visit history is strongly indicative of future browsing interests.
Most existing sequential recommendation methods focus on Markov models
and tensor-factorized extensions; while these methods do capture
sequential context, they cannot easily capture long-term historical
dependencies due to their intrinsic Markov assumption.  In this paper,
we propose and evaluate a Deep Recurrent Neural Network (DRNN) for
sequential item recommendation that uses gated recurrent memory units
(GRUs) to compress user interaction histories and learn long-term
dependencies in this compressed representation.  In a comparative
evaluation on educational and web datasets, we show that DRNN
outperforms a deep network without long short-term memory as well as a
state-of-the-art tensor factorized personalized Markov chain (FPMC)
approach, thus demonstrating the importance of learning long-term dependencies
...
dependencies are generally impractical due to the memory and
...
GRU-based DRNN for sequential recommendation in Figure~\ref{fig:ModelArch} and 
demonstrate that the DRNN outperforms variants of the state-of-the-art FPMC 
model.  We also show that removing the DRNN model's memory leads to performance

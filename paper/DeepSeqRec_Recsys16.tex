...
\maketitle
\begin{abstract}
Many recommendation settings have a sequential nature.  For example, in online education tutoring, next problem recommendation that maximizes learning should take a student's historical progress into account. Similarly, online web browsing is highly contextual and a user's site visit history is strongly indicative of future browsing interests. Most existing sequential recommendation methods focus on Markov models and tensor-factorized extensions; while these methods do capture sequential context, they cannot easily capture long-term historical dependencies due to their intrinsic Markov assumption.  In this paper, we propose and evaluate a Deep Recurrent Neural Network (DRNN) for sequential item recommendation that uses gated recurrent memory units (GRUs) to compress user interaction histories and learn long-term dependencies in this compressed representation.  In a comparative evaluation on educational and web datasets, we show that DRNN outperforms a deep network without long short-term memory as well as a state-of-the-art tensor factorized personalized Markov chain (FPMC) approach, thus demonstrating the importance of learning long-term dependencies in sequential recommendation.  
\end{abstract}
...
\keywords{Sequential Recommendation; Recurrent Neural Networks; Long Short-term Memory}
...
collectively influence where the user may want to visit next.  While
existing state-of-the-art methods for general sequential
recommendation range from factored personalized Markov
chains (FPMC)~\cite{Rendle2010} to metric embeddings~\cite{Feng2015,Wu2013},
they all share the common limitation that they only model pairwise, but
not higher-order, historical dependencies.
%
...
%<<<<<<< HEAD
%In this paper, our primary hypothesis is that long-term dependencies are
%important for making relevant sequential recommendations. Yet,
%extending existing methods to make them higher-order Markov to capture such dependencies is generally impractical due to the memory and computational requirements of learning higher-order (factorized) tensors.  However, recent developments in deep recurrent neural networks (DRNNs) provide an efficient linear space and time method of learning long-term dependencies via gated recurrent memory units (GRUs)~\cite{Cho2014}; specifically, GRUs do not aim to model all possible higher-order historical interactions, but instead learn how to embed histories into a latent space and what information to propagate in a linear sequential fashion.  To this end, we propose a GRU-based DRNN for sequential recommendation and demonstrate that the DRNN outperforms variants of the Markov chain models and also show that removing the model's memory leads to performance similar to FPMC. Our results thus support our hypothesis that learning long-term dependencies can improve sequential recommendations.
%
...
important for making relevant sequential recommendations.  Yet,
extending existing methods for modeling higher-order 
dependencies is generally impractical due to the memory and
...
GRU-based DRNN for sequential recommendation in Figure~\ref{fig:ModelArch} and demonstrate that the DRNN outperforms variants of the state-of-the-art FPMC model.  We also show that removing the DRNN model's memory leads to performance similar to FPMC.  Our results thus support our hypothesis that learning long-term dependencies can improve sequential recommendations.
%>>>>>>> 962ccc6b67549a4a0afaa6977474614b6345bce4
...
\centering
	...
	\caption{The model's memory is composed of Gated Recurrent Units (GRUs) that selectively update or reset their hidden states via two gates for each cell $k$: the update gate $z_k$ controls the level at which the internal state $h$ is replaced by a new hidden state $\tilde{h}$ (a weighted average). The reset gate $r_k$ controls how much of the previous hidden state to forget. %Although sometimes shown as discrete gates, the updates/resets are linear interpolations using parameters learnt from data. 
	After a user's interaction history is fed into the cells, the  hidden state is relayed to higher layers.
	}
	\label{fig:GRU}
\end{figure}
...
Turning our attention to the other methods, the DNN and FPMC perform similarly; FPMC achieves slightly better scores on MSWeb, and DNN is better on Assistments'15. However, the differences were not statistically significant. Interestingly, the baseline MC outperformed both FPMC and DNN on Assistments'15 (significant at $\alpha = 1\%$). Under Markovian conditions, it is possible that the dense  global MC matrix (48\% filled) lent itself well to the relatively structured setting of next problem-set recommendation, counteracting any potential benefits from user personalization. %That said, the higher DRNN scores suggest the benefit of taking sequential order into consideration when making predictions in this domain.
...
This work presented and empirically evaluated DRNN, a deep recurrent neural model for sequential recommendation that leverages on GRU memory cells to selectively retain information from a user's interaction history. Experiments on two datasets showed that the DRNN outperformed a memoryless deep neural network and a state-of-the-art tensor factorized Markov model. These results lend support to the notion that capturing long-term dependencies is important for making relevant recommendations. Moving forward, it would be interesting to apply DRNN to more specialized sequential domains such as point-of-interest (POI) recommendation, which  exploit geographic-specific properties, e.g., distance from current location~\cite{Ye2011}.


%Moving forward, the DRNN can be extended in several interesting ways; for example, it is straightforwardly applied to cold-start settings (e.g., for book recommendation~\cite{Pera2013}) by replacing the embedding lookups with inputs for relevant side-information (book content and user features). If needed, feature  representations can been learnt via deep convolutional nets trained on unstructured data. 

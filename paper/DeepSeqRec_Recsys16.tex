...
\maketitle
\begin{abstract}
Many recommendation settings have a sequential nature.  For example, in online education tutoring, student knowledge is dynamic and recently completed problem sets are relevant for recommending future problem sets. Similarly, online web browsing sessions are highly contextual and historical user site visits are indicative of future browsing interests.  Most existing sequential recommendation methods focus on Markov models and tensor-factorized extensions; while these methods do capture sequential context, they cannot easily capture long-term dependencies due to their Markov assumption. We hypothesize that long-term dependencies, and not just the last user interaction, is critical for forming accurate recommendations. In this paper, we propose and evaluate a Deep Recurrent Neural Network (DRNN) (Fig. \ref{fig:ModelArch}) for sequential item recommendation that uses gated recurrent memory units (GRUs) to learn latent contexts that compress user interaction histories.  In a comparative evaluation on educational and web datasets, we show that DRNN outperforms a deep network without long short-term memory as well as a state-of-the-art tensor factorized personalized Markov chain (FPMC), thus demonstrating the importance of learning long-term dependencies in sequential recommendation.
\end{abstract}
